...
export default function AccountPassesSection({ 
        ...
        <div className="text-center">
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-4 mb-3">
              ...
              <div>
                ...
                <p className="text-sm text-foreground">
                  {selectedPass.duration_days} {selectedPass.duration_days === 1 ? 'Day' : 'Days'}
                </p>
              </div>
            </div>
            ...
            <div className="text-xs text-muted-foreground space-y-1">
              ...
              <p>Expires: {selectedPass.duration_days ? `${selectedPass.duration_days} days from purchase` : 'N/A'}</p>
            </div>
  ...
  return (
    // Simplified Passes List View - Small Cards with only Name, Price, Status
    <div className="space-y-3">
      {userPasses.map((pass, index) => (
        <div
          key={`${pass.id}-${index}`}
          onClick={() => onPassSelect(pass)}
          className="cursor-pointer transition-colors duration-200 p-3 rounded-lg bg-primary/5 hover:bg-primary/10 border border-transparent hover:border-primary/20"
        >
